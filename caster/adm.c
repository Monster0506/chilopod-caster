...
int admsrv(struct ntrip_state *st, const char *method, const char *root_uri, const char *uri, int *err, struct evkeyvalq *headers) {
	...
	if (!strcmp(method, "POST")) {
		...
		if (!st->content_type || !strcmp(st->content_type, "application/x-www-form-urlencoded")) {
			/*
			 * A urlencoded body may be empty for a bare POST (e.g. auth via
			 * header only, or a missing Content-Type); NULL is handled fine.
			 */
	...
	if (req->hash) {
		...
		const struct uri_calls calls[] = {
			...
			{"/api/v1/sources/edit", "POST", api_edit_source_json},
